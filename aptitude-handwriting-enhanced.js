/**
 * Aptitude Handwriting Recognition - Enhanced
 * Kid-friendly enhancement for counting and logic puzzles
 * Uses TensorFlow.js for digit recognition
 */

let aptitudeEnhancedReady = false;

/**
 * Initialize enhanced aptitude handwriting recognition
 */
async function initializeAptitudeML5() {
    try {
        console.log('🧩 Initializing aptitude handwriting recognition...');

        if (typeof tf !== 'undefined') {
            await tf.ready();
        }

        aptitudeEnhancedReady = true;

        console.log('✅ Aptitude handwriting recognition ready!');
        console.log('🎨 Enhanced recognition for counting & logic puzzles');

        return true;
    } catch (error) {
        console.error('❌ Error initializing aptitude handwriting:', error);
        return false;
    }
}
...
function showAptitudeFeedback(canvas, feedback) {
    let feedbackEl = canvas.parentElement.querySelector('.aptitude-enhanced-feedback');

    if (!feedbackEl) {
        feedbackEl = document.createElement('div');
        feedbackEl.className = 'aptitude-enhanced-feedback';
...
console.log('🧩 Aptitude handwriting helper loaded');
