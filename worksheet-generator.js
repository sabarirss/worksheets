...
function showMathLevels() {
    ...
    // Go directly to progress map (tabs allow switching between operations)
    if (typeof showProgressMap === 'function') {
        showProgressMap('math', 'addition');
    } else {
        // Fallback if progress-map.js not loaded
        const subjectSelection = document.querySelector('.subject-selection');
        const operations = document.getElementById('math-operations');
...
async function loadOperationWorksheet(operation) {
    ...
    if (typeof getAccessiblePages === 'function') {
        ...
        // Check if this is an adaptive (personalized) assignment
        if (access.adaptive && access.assignment) {
            isAdaptiveMode = true;
            currentAdaptiveAssignment = access.assignment;
            console.log('Adaptive mode: loading personalized worksheets');
        } else {
            isAdaptiveMode = false;
            currentAdaptiveAssignment = null;
        }
...
let isAdaptiveMode = false;          // Whether current worksheet is from adaptive engine
let currentAdaptiveAssignment = null; // Store full assignment doc for adaptive mode
...
function loadWorksheetByPage(operation, absolutePage) {
    ...
    // Adaptive mode: load problems from assignment doc instead of generating
    if (isAdaptiveMode && currentAdaptiveAssignment) {
        loadAdaptiveWorksheet(operation, absolutePage);
        return;
    }
...
/**
 * Load adaptive worksheet from assignment doc (problems pre-generated by server)
 * @param {string} operation - Math operation
 * @param {number} pageNumber - Page number (1-7) within the adaptive assignment
 */
function loadAdaptiveWorksheet(operation, pageNumber) {
    const assignmentPages = currentAdaptiveAssignment.math.pages;
    const pageData = assignmentPages.find(p => p.pageNumber === pageNumber);
    if (!pageData) {
        console.error(`Adaptive page ${pageNumber} not found in assignment`);
        return;
    }
    ...
    currentWorksheet = {
        operation,
        ageGroup: currentAdaptiveAssignment.math.ageGroup || selectedAgeGroup,
        difficulty: 'adaptive',
        page: pageNumber,
        config: {
            name: `${opName} - Personalized Practice`,
            description: 'Problems selected based on your learning profile',
            problemCount: pageData.problems.length
        },
        problems: pageData.problems.map(p => ({
            a: p.a,
            b: p.b,
            display: p.display || `${p.a} ${symbol} ${p.b}`,
            type: p.type
        })),
        answers: new Array(pageData.problems.length).fill(''),
        adaptive: true
    };
...
async function submitWorksheet() {
    ...
    for (let i = 0; i < totalProblems; i++) {
        ...
        if (usePencil) {
            try {
                // Use recognizeNumber for multi-digit support (10, 12, 25, etc.)
                const result = typeof recognizeNumber === 'function'
                    ? await recognizeNumber(answerElement)
                    : await recognizeDigit(answerElement);
                const value = result.number !== undefined ? result.number : result.digit;
                answers.push(result.isEmpty ? null : value);
            } catch {
                answers.push(null);
            }
        } else {
    ...
    try {
        let result;
        if (currentWorksheet.adaptive) {
            // Adaptive worksheets: validate against stored problems
            const validateAdaptive = firebase.app().functions('europe-west1').httpsCallable('validateAdaptiveSubmission');
            result = await validateAdaptive({
                childId: child ? child.id : null,
                operation: operation,
                pageNumber: currentAbsolutePage,
                answers: answers,
                elapsedTime: elapsedTime
            });
        } else {
            // Standard worksheets: validate against deterministic seed
            const validateMath = firebase.app().functions('europe-west1').httpsCallable('validateMathSubmission');
            result = await validateMath({
                childId: child ? child.id : null,
                operation: operation,
    ...
    for (let i = 0; i < totalProblems; i++) {
        ...
        if (answers[i] === null || answers[i] === undefined || answers[i] === '') {
            ...
            if (problemEl) problemEl.classList.add('answer-correct');
            if (typeof playSound === 'function') playSound('correct');
        } else {
            feedbackElement.textContent = '✗ ' + (typeof getEncouragement === 'function' ? getEncouragement(false) : 'Try again!') + ` (Answer: ${fb.expected})`;
            feedbackElement.style.color = '#f44336';
            feedbackElement.style.fontWeight = 'bold';
            answerElement.style.borderColor = '#f44336';
            answerElement.style.borderWidth = '3px';
            const problemEl = answerElement.closest('.problem') || answerElement.parentElement;
            if (problemEl) problemEl.classList.add('answer-incorrect');
